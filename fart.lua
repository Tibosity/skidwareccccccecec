...
local library = {
    ...
    folders = {
        main = "AntarcticaWare",
        assets = "AntarcticaWare/Images",
        configs = "AntarcticaWare/Configs"
    },
    shared = {
        initialized = false,
        fps = 0,
        ...
        function window:Watermark(info)
            ...
            utility:Connection(rs.RenderStepped, function(FPS)
                ...
                if (tick() - Tick) > 0.15 then
                    watermark_title.Text = string.format("$$$$$ AntarcticaWare $$$$$ || UID : %u || Ping : %s || Fps : %u", 1, library.shared.ping, library.shared.fps)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
